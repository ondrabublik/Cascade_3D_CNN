...
class ReflectPadding3D(tf.keras.layers.Layer):
...
class UNetDev:
    ...
    def conv3d_with_reflect_padding(self, x, filters, kernel_size, activation):
        pad = kernel_size[0] // 2

        x = ReflectPadding3D(pad)(x)
    ...
    def build(self):
        ...
        for i in range(1, self.deep):
            conv[i - 1] = self.conv3d_with_reflect_padding(layer, nChannels[i], frame, self.act)
            layer = MaxPooling3D(pool_size=poolFrame)(conv[i - 1])

        encoded = self.conv3d_with_reflect_padding(layer, self.deep * self.nChannel, frame, self.act)
        ...
        for i in range(self.deep - 1, 0, -1):
            ...
            conc = self.conv3d_with_reflect_padding(layer, nChannels[i], frame, self.act)
            layer = concatenate([conc, conv[i - 1]])

        out = self.conv3d_with_reflect_padding(layer, self.dimOut, frame, self.actOut)

...
class UNetDev:
    ...
    def build(self):
        ...
        input6 = input[..., 6:7]
        input7 = dataNormalization(input[..., 7:8], self.scales['minVel'], self.scales['maxVel'])
        ...
        input10 = dataNormalization(input[..., 10:11], self.scales['minP'], self.scales['maxP'])

        uMesh = input[..., 3:4]
        vMesh = input[..., 4:5]
        wMesh = input[..., 5:6]

        layer = Concatenate(axis=4)([input0, input1, input2, input3, input4, input5, input6, input7, input8, input9, input10])
        # encoder
        conv = [None] * self.deep
